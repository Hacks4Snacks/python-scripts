...
try:
    ...
    sys.exit(
        "[!] Install the xlsx writer library as root or through sudo: pip install xlsxwriter")
    ...
    def generate_xlsx(self):
        ...
        for key, value in self.hosts_dict.items():
            try:
                ...
                if self.verbose > 3:
                    print("[!] An error occurred parsing host ID: %s for Worksheet 1") % (
                        key)
            try:
                if row % 2 != 0:
                    temp_format = format2
        ...
        try:
            ...
            sys.exit(
                "[!] Permission to write to the file or location provided was denied")
