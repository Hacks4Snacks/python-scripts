    ...
    def run(self):
        ...
        for host in root.iter('host'):
            ...
            for item in host.iter('port'):
                ...
                services.append(
                    [hostname_list, address, protocol, port, service, hwaddress, state])
        hostname_list = []
        for i in range(0, len(services)):
            ...
            hwaddress = service[5]
            state = service[6]
            self.hosts[i] = [hostname, address, protocol,
                             port, serv_name, hwaddress, state]
            if self.verbose > 2:
                print("[+] Adding %s with an IP of %s:%s with the service %s") % (
                    hostname, address, port, serv_name)
        if self.hosts:
            if self.verbose > 4:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-x", "--xml", type=str,
                        help="Generate a dictionary of data based on a NMAP XML import, more than one file may be passed, separated by a comma", action="store", dest="xml")
    parser.add_argument("-f", "--filename", type=str, action="store", dest="filename",
                        default="xml_output", help="The filename that will be used to create an XLSX")
    parser.add_argument("-s", "--simple", action="store_true", dest="simple",
                        help="Format the output into a simple excel product, instead of a report")
    ...
    # Temporary dictionary, which holds returned data from specific instances
    hosts_temp = {}
    # Dictionary, which holds the combined returned dictionaries
    hosts_dict = {}
    # The dictionary, which holds the unique values from all processed XMLs
    processed_hosts = {}
